...
URL を `http://localhost:4321/guides/example` に書き換えるか、`Examples Guides >` をクリックします。    
...
#### ビルド、デプロイ
GitHub リポジトリを開き、Action ボタンを押します。これでデプロイが完了します。  
![](figs/20260411-repo_actions.jpg)

もし名前変更後にデプロイでエラーが出た場合は、master ブランチしかデプロイ対象になっていない可能性があります。  

リポジトリの Setting > Environments > github-pages > Deployment branches に移動します。  
ドロップダウンを **No restriction（制限なし）** にするか、main ブランチを登録してください。  
![](figs/20260411-repo_env.jpg)

デプロイができたら `https://<username>.github.io/` にアクセスしてみましょう。  
